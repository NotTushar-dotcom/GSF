      ...
      <main className="pt-24 min-h-screen bg-[#FDFAF7]">
        ...
        <section className="section-container pb-16">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            {[
              { step: "01", icon: Lightbulb, title: "Student lists idea", desc: "Submit startup, equity offer, and traction metrics" },
          ...
          <div className="section-container">
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              ...
              <select id="stage-filter" className="input sm:w-36">
                {["All", "Idea", "Pre-seed", "Seed"].map((s) => <option key={s}>{s}</option>)}
              </select>
                ...
                <div key={v.id} className="card card-hover p-6 flex flex-col gap-4 bg-white">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="badge badge-blue text-xs">{v.stage}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${v.daysLeft <= 7 ? 'text-red-600 bg-red-50 border border-red-100' : 'text-[#8A95A3] bg-[#F3E3D0]'}`}>
                        {v.daysLeft}d left
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[{ label: "Seeking", value: v.seeking }, { label: "Equity", value: v.equity }, { label: "Traction", value: v.traction }].map(({ label, value }) => (
                      <div key={label} className="bg-[#F7F2EC] border border-[#D2C4B4] rounded-xl p-3">
                        ...
                        <div className="text-sm font-semibold text-[#1A2332]">{value}</div>
                      </div>
                    ))}
                  </div>
